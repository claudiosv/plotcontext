...
class PlotContext:
    ...
    def __init__(
        ...
        *,
        ...
        rc_figx, rc_figy = rc_params.get("figure.figsize") or rcParams.get(
            "figure.figsize", (10.0, 5.625)
        )
        ...
        self.dpi = dpi or rcParams.get("figure.dpi", 300)
    ...
    def _setup_styles(self) -> None:
        ...
        contexts: list[AbstractContextManager] = [
            sns.plotting_context(context=self.plot_context, font_scale=self.font_scale),
            sns.axes_style(self.style),
            self._palette,
            plt.rc_context(self.rc_params),
        ]
    ...
    def _close(self) -> None:
        ...
        if not backend.startswith("module://ipympl"):
            plt.close(
                "all"
    ...
    def _draw(self) -> None:
        ...
        if self.grid:
            self.ax.grid(visible=True, linestyle="dashed", alpha=0.6, axis=self.grid)
    ...
    def save_fig(self, output_path: Path | str, name: str) -> None:
        ...
        """StrMethodFormatter (Recommended): Uses Python's modern {} string formatting syntax. It is the cleanest way to format numbers without writing a custom function.

Python
import matplotlib.ticker as ticker

# Formats as currency: $1,234.50
ax.yaxis.set_major_formatter(ticker.StrMethodFormatter(f'${x:,.2f}'))
FormatStrFormatter: The older sibling to StrMethodFormatter. It uses the older % style formatting (like C-style printf).

Python
    # Formats to two decimal places: 1.23
    ax.yaxis.set_major_formatter(ticker.FormatStrFormatter('%.2f'))
    ```

### 2. For Maximum Customization
When your formatting logic is too complex for a simple string template (e.g., changing formatting based on the value's size, or mapping specific numbers to words).

*   **`FuncFormatter`:** Allows you to pass a custom Python function to format the ticks. The function must accept two arguments: `x` (the tick value) and `pos` (the tick position).

```python
    def millions(x, pos):
        return f'{x*1e-6:1.1f}M'

    # Converts 1,500,000 to "1.5M"
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(millions))
    ```

### 3. For Specialized Data Types
Matplotlib has built-in formatters for common data representations so you don't have to write the math yourself.

*   **`PercentFormatter`:** Automatically converts numbers to percentages and appends the `%` sign. You can tell it what your maximum value is (e.g., if your data is 0.0 to 1.0, or 0 to 100).

```python
    # Data is fractions (0.0 to 1.0) -> formats as 0% to 100%
    ax.yaxis.set_major_formatter(ticker.PercentFormatter(xmax=1.0))
    ```
*   **`EngFormatter`:** Excellent for scientific or engineering plots. It automatically appends SI prefixes (k, M, G, m, µ, etc.) based on the magnitude of the number and can include a unit.

```python
    # Converts 1500 to "1.5 k Hz"
    ax.yaxis.set_major_formatter(ticker.EngFormatter(unit='Hz'))
    ```

### 4. For Logarithmic Scales
If you are using `ax.set_yscale('log')`, standard formatters usually fail to look good.

*   **`LogFormatterMathtext`:** Formats log ticks using standard scientific notation with proper superscripts (e.g., $10^3$ instead of $1e3$), which looks much better in publications.
*   **`LogFormatterSciNotation`:** Similar, but uses the `1.0e3` format explicitly."""
